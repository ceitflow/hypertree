...
import { ExtentLimiter } from '../plugins/limiter';
...
export function PhysicsInput({ config, physics, frameStart, transform: t, physicsTransform: pt, extent, viewport }: State) {
  ...
  return {
    addForceWithLimitToExtent: (forces: Vector4, dx: number, dy: number): void => {
      const { xForce, yForce, isConstrainedX, isConstrainedY } = ExtentLimiter(dx, dy, t, viewport, config.viewportPadding, extent);
      forces[0] += xForce;
      forces[1] += yForce;
      forces[2] = xForce;
      forces[3] = yForce;
      if (!isConstrainedX && !isConstrainedY) clearLimiterForces(forces);
