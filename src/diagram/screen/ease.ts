...
export class Ease {
  ...
  static Inverse(f: EaseFunction, resultT: number, c: number, d: number) {
    // todo this is approximation (bisection method), not for every function
    let low = 0;
    let high = 1;

    while (high - low > 1e-4) {
      const mid = (low + high) / 2;
      const value = f(mid, c, d);
      if (value < resultT) low = mid;
      else high = mid;
    }
    return (low + high) / 2;
  }
  ...
  static inElastic(t: number, c: number, d: number) {
    ...
    if (c < Math.abs(c)) {
      s = p / 4;
    } else s = (p / (2 * Math.PI)) * Math.asin(1);
    return -(c * Math.pow(2, 10 * (t -= 1)) * Math.sin(((t * d - s) * (2 * Math.PI)) / p));
  }
  ...
  static outElastic(t: number, c: number, d: number) {
    ...
    return c * Math.pow(2, -10 * t) * Math.sin(((t * d - s) * (2 * Math.PI)) / p) + c;
  }
  ...
  static inOutElastic(t: number, c: number, d: number) {
    ...
    if (t < 1) return -0.5 * (c * Math.pow(2, 10 * (t -= 1)) * Math.sin(((t * d - s) * (2 * Math.PI)) / p));
    return c * Math.pow(2, -10 * (t -= 1)) * Math.sin(((t * d - s) * (2 * Math.PI)) / p) * 0.5 + c;
  }
